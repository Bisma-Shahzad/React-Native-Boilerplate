...
export const ChatScreen = ({ route }) => {
  ...
  console.log('videoCallingaaaaaaaa', videoCallingData);
    ...
    try {
      ...
      if (videoCalls) {
        ...
        if (isBusy) {
          ...
          const isBusyWithAuthUser = Object.values(videoCalls).some(
            call => call.callerId === AuthReducerId || call.recieverId === AuthReducerId
          );
          ...
          if (isBusyWithAuthUser) {
            ...
            console.log('User is busy with Auth user. Cannot initiate video call.');
      ...
      const messageRef = database()
        ...
        .then(async () => {
          ...
          const snapshot = await database().ref(`videocall/${videoCallId}`).once('value');
  ...
  const handleVideoCallEnd = () => {
    ...
    messagesRef.child(`${videoCallingData.id}`).remove()
      .then(() => {
    ...
    <>
      <SafeArea>
        {loader ? (
          ...
          <VideoCall setVideoCalling={setVideoCalling} onCallEnd={handleVideoCallEnd} callData={videoCallingData} />
        ) : (
          <View
            style={{
              flex: 1,
            ...
            <View style={{ backgroundColor: 'white' }}>
              <Head
                head={userBio.is_active ? userBio.display_name : 'No User Found'}
                // backName={backNames}
            ...
            >
              ...
              <View
                ...
                <TouchableOpacity onPress={userBio.is_active ? sendMessage : null}>
                  <Image
